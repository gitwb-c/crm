...
const worker_4 = new bullmq_1.Worker("evolutionQueue", async (job) => {
...
worker_4.on("completed", (job) => console.log(`${job.id} processado no worker 4  -  ${new Date().toLocaleString()}`));
worker_4.on("failed", (job, err) => console.log(`${job?.id} falhou no worker 4  -  ${new Date().toLocaleString()}`, err));
