...
const worker_3 = new bullmq_1.Worker("evolutionQueue", async (job) => {
...
worker_3.on("completed", (job) => console.log(`${job.id} processado no worker 3  -  ${new Date().toLocaleString()}`));
worker_3.on("failed", (job, err) => console.log(`${job?.id} falhou no worker 3  -  ${new Date().toLocaleString()}`, err));
