...
worker_2.on("completed", (job) =>
  console.log(
    `${job.id} processado no worker 2  -  ${new Date().toLocaleString()}`
  )
);

worker_2.on("failed", (job, err) =>
  console.log(
    `${job?.id} falhou no worker 2  -  ${new Date().toLocaleString()}`,
    err
  )
);
